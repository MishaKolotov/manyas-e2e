...
projects = LOCALES.flatMap((loc) =>
  DEVICES.flatMap((dev) => ENGINES.map((eng) => buildProject(loc, dev, eng))),
);
```
...
3. Для **Sheet1** и **Sheet3** (с явными ключами):
   ...
   - **Дубликаты ключей между Sheet1 и Sheet3.** Эмпирическая проверка показала, что в реальном xlsx Sheet3 — это «новая редакция» таблицы, и приложение использует строки из Sheet3 для всех конфликтующих ключей (`infoAlmostDone_subtitle`, `howQuicklyLossWeight_title`, `focus_title`). Поэтому импортёр **обрабатывает Sheet3 перед Sheet1** и применяет правило:
     - same-value duplicate → silent passthrough, ключ логируется в `_meta.duplicateKeys` (для аудита, чтобы знать, что в xlsx живёт мусор).
     - different-value duplicate → Sheet3 wins (она первая в обходе → её значение остаётся в `accepted[key]`, Sheet1-значение отбрасывается); ключ логируется в `_meta.duplicateKeys` с маркером `OVERRIDDEN_BY_SHEET3`. Никаких throw в production-режиме — это шум команды локализации, не баг.
     - Если когда-нибудь появятся duplicates **внутри одного sheet** (Sheet1 содержит ключ дважды, или Sheet3 содержит ключ дважды) — импортёр **падает с ошибкой**, потому что это уже не "две версии таблицы", а реальный баг в xlsx.
